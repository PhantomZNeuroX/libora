...
let App = function (el) {
    ...
    this.qs("button.next").addEventListener("click", () => {
        console.log('Rendition Next')
        try {
            var time_spent = Date.now()-window.time_spent;
            console.log(time_spent)
            if (time_spent > 30000 && time_spent < 120000){
                window.time_spent = time_spent;
                window.tim = time_spent;
                //sendTime(time_spent, window.words, window.users);
                let location = this.state.rendition.location;
                //this.state.rendition.currentLocation()
                console.log(location);
                var start = location['start'];
                var end = location['end'];
                //var startCFI = console.log(location['start']['cfi']);
                //var endCFI = console.log(location['end']['cfi']);
                //var startCfi = startCFI
                //var endCfi = endCFI
                //var cfiBase = startCfi.replace(/!.*/, '') 
                //var cfiStart = startCfi.replace(/.*!/, '').replace(/\)$/, '')
                //var cfiEnd = endCfi.replace(/.*!/, '').replace(/\)$/, '')
                //var cfiRange = `${cfiBase }!,${cfiStart },${cfiEnd })`
                //var cfiRange = 'epubcfi(/6/26[ch08]!,/4/28[page_107]/16/2/1:0,/4/36[page_109]/2/2/1:156)'
...
function SubForm (){
    $.ajax({
        url: 'http://localhost:8000/book/progress/report',
        type: 'post',
...
function sendTime(){
    const url = 'http://localhost:8000/book/progress/report'

    /*
    var query = {
        'csrfmiddlewaretoken':window.csrf,
        'words':window.pageText,
        'time':window.tim
    }
    ...
    */
    ...
    fo.setAttribute("action", "/book/progress/report")
    fo.setAttribute("method","POST")
    fo.setAttribute("id", "formInv")

    var csrf = document.createElement("input");
    csrf.setAttribute("name","csrfmiddlewaretoken")
    csrf.setAttribute("value", window.csrf)
    csrf.setAttribute("type", 'hidden')
    fo.appendChild(csrf)

    var wor = document.createElement("input");
    wor.setAttribute("name","words")
    wor.setAttribute("value", window.pageText)
    fo.appendChild(wor)

    var timee = document.createElement("input");
    timee.setAttribute("name","time")
    timee.setAttribute("value", window.tim)
    fo.appendChild(timee)

    var user = document.createElement("input");
    user.setAttribute("name","user")
    user.setAttribute("value", window.users)
    fo.appendChild(user)

    var book = document.createElement("input");
    book.setAttribute("name","book")
    book.setAttribute("value", window.book)
    fo.appendChild(book)
...
App.prototype.doOpenBook = function () {
    ...
    */
    ...
    window.onload = openn;
    function openn() {
        this.doBook('http://localhost:8000/media/it-ends-with-us_Snj8Srh_2_sifLaDW.epub');
...
App.prototype.doReset = function () {
    ...
    this.qs(".book").innerHTML = '<div style="visibility: hidden;" class="empty-wrapper"><div class="empty"><div class="app-name">Loading..</div><div class="message"><a href="javascript:ePubViewer.doOpenBook();" class="big-button" id="launch">Open a Book</a></div></div></div>';
